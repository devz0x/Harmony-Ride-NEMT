const PAT = "sbp_e22867ad37f18f94172b69ceb6f4edd066e2c176";
const URL = "https://api.supabase.com/v1/projects/nxraigfxeofzlnnafkak/database/query";
...
async function q(sql, label) {
  ...
  if (body.includes('"message"') && !body.startsWith('[')) {
    process.stdout.write(`  ERR ${label}: ${body}\n`);
  } else {
    process.stdout.write(`  OK  ${label}  ${body}\n`);
  }
}
...
await q(`create table if not exists invoice_line_items (
...
for (const t of ["drivers","patients","vehicles","trips","invoices","invoice_line_items"]) {
  await q(`alter table ${t} enable row level security`, `RLS ${t}`);
  await q(`do $$ begin
    if not exists (select 1 from pg_policies where tablename='${t}' and policyname='anon read ${t}') then
      execute 'create policy "anon read ${t}" on ${t} for select using (true)';
    end if;
  end $$`, `policy ${t}`);
}

process.stdout.write("\n=== Seeding drivers ===\n");
await q(`insert into drivers (id,name,phone,email,license,license_exp,background,training,vehicle,status,rating,trips,join_date,photo) values
...
on conflict (id) do nothing`, "drivers");

process.stdout.write("\n=== Seeding patients ===\n");
await q(`insert into patients (id,name,dob,phone,email,address,insurance,member_id,transport,conditions,notes,total_trips,last_trip,status) values
...
on conflict (id) do nothing`, "patients");

process.stdout.write("\n=== Seeding vehicles ===\n");
await q(`insert into vehicles (id,type,make,year,plate,vin,capacity,mileage,last_service,next_service,inspection,status,driver,features) values
...
on conflict (id) do nothing`, "vehicles");

process.stdout.write("\n=== Seeding trips ===\n");
await q(`insert into trips (id,patient,dob,phone,pickup,destination,date,time,return_time,type,transport,driver,vehicle,insurance,status,notes) values
...
on conflict (id) do nothing`, "trips");

process.stdout.write("\n=== Seeding invoices ===\n");
await q(`insert into invoices (id,invoice_date,due_date,bill_to,status,paid_date,denial_reason,payment_terms) values
...
on conflict (id) do nothing`, "invoices");

process.stdout.write("\n=== Seeding line items ===\n");
await q(`insert into invoice_line_items (invoice_id,num,date,service,description,qty,rate,amount) values
...
on conflict do nothing`, "line items");

process.stdout.write("\n=== Row counts ===\n");
await q(`select 'drivers' as tbl, count(*)::text as n from drivers
union all select 'patients', count(*)::text from patients
union all select 'vehicles', count(*)::text from vehicles
union all select 'trips', count(*)::text from trips
union all select 'invoices', count(*)::text from invoices
union all select 'invoice_line_items', count(*)::text from invoice_line_items`, "counts");
