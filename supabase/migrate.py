...
URL = "https://api.supabase.com/v1/projects/nxraigfxeofzlnnafkak/database/query"
...
def q(sql, label):
    ...
    try:
        ...
        print(f"  ERR {label}: {e.read().decode()}")
...
q("""create table if not exists trips (
...
q("""create table if not exists invoice_line_items (
...
print("\n=== Enabling RLS ===")
for t in ["drivers","patients","vehicles","trips","invoices","invoice_line_items"]:
    q(f"alter table {t} enable row level security", f"RLS {t}")
    q(f"""do $$ begin
...
print("\n=== Seeding drivers ===")
q("""insert into drivers (id,name,phone,email,license,license_exp,background,training,vehicle,status,rating,trips,join_date,photo) values
...
print("\n=== Seeding patients ===")
q("""insert into patients (id,name,dob,phone,email,address,insurance,member_id,transport,conditions,notes,total_trips,last_trip,status) values
...
print("\n=== Seeding vehicles ===")
q("""insert into vehicles (id,type,make,year,plate,vin,capacity,mileage,last_service,next_service,inspection,status,driver,features) values
...
print("\n=== Seeding trips ===")
q("""insert into trips (id,patient,dob,phone,pickup,destination,date,time,return_time,type,transport,driver,vehicle,insurance,status,notes) values
...
print("\n=== Seeding invoices ===")
q("""insert into invoices (id,invoice_date,due_date,bill_to,status,paid_date,denial_reason,payment_terms) values
...
print("\n=== Seeding line items ===")
q("""insert into invoice_line_items (invoice_id,num,date,service,description,qty,rate,amount) values
...
q("""select 'drivers' as tbl, count(*)::text from drivers
...
union all select 'invoice_line_items', count(*)::text from invoice_line_items""", "row counts")
